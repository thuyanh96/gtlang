...
tokens_line1 get_last_ele(tokens_arr _arr) {
	tokens_line1 _ele;
	if (!_arr.empty())
...
tokens_arr rtrim_all_lines_arr(tokens_arr $content) {
//	long $k0;
	tokens_line1 $line;
//	long $k1;
	token $token;
...
string tokens_arr_to_str1(tokens_line $tokens_arr) {
	string $return;
	token $token;
	$return = "";
	for (auto [$key_rtt, $token] : $tokens_arr) {
		$return += $token.$0;
	}
	return $return;
}
tokens_line_arr explode_tokens(token $del, tokens_line $tokens_arr) {
...
tokens_line_arr explode_tokens(string $del, tokens_line $tokens_arr) {
	tokens_line_arr $return;
	long $k;
	token $token;
	$return = { { 0, { } } };
...
tokens_line remove_spaces_from_tokens_arr(tokens_line $tokens_arr) {
	tokens_line $return;
	token $token;
	$return = { };
	for (auto [$key_rtt, $token] : $tokens_arr) {
...
Mixed remove_spaces_from_tokens_arr(Mixed $tokens_arr) {
	Mixed $return;
	for (auto [$key_rtt, $token] : $tokens_arr.valueArray) {
		if (!($token[1] == "") && !($token[1] == "spaces"))
			$return.push($token);
	}
	return $return;
...
void new_line(tokens_arr &$arr, long &$line) {
	$line++;
	if (!isset($arr, $line))
		$arr[$line] = { { }, 0 };
}
tokens_arr tokenize(string $content) {
	tokens_arr $output_arr;
	long $len;
	long $line;
	long $block_lv;
	long $i;
...
Mixed get_arr_between_brackets_mixed(string $open_bracket, Mixed $tokens_arr,
	...
	if ($open_bracket == "[")
		$close_bracket = "]";
	$lv = 1;
	for (auto [$k, $v] : $tokens_arr.valueArray) {
	...
	return array_slicem($tokens_arr, $pos_of_open_bracket + 1,
			$k - $pos_of_open_bracket - 1);
}
tokens_line get_arr_between_brackets_tl(string $open_bracket,
		tokens_line $tokens_arr, long $pos_of_open_bracket) {
	string $close_bracket;
	long $lv;
	long $k;
	token $v;
	...
	return array_slice($tokens_arr, $pos_of_open_bracket + 1,
			$k - $pos_of_open_bracket - 1);
}
arr_ll find_all_token_at_level0_by_type(string $token_type_to_compare,
		tokens_line $tokens_arr) {
	long $lv1;
	long $lv2;
	arr_ll $return;
	//long $k;
...
Mixed get_function_and_params_lv0(tokens_line $tokens_arr) {
	Mixed $return;
	...
	if (sizeof($all_func_names_pos) != 1)
		return Mixed();
	$return.push("name", $tokens_arr[$all_func_names_pos[0]].$0);
...
getline_subblock_t getline_subblock(tokens_arr $tokens_arr,
		long $header_line_i) {
	long $header_lv;
	tokens_arr $tmp_arr;
	long $i;
	...
	for ($i = 0; $i < (long) sizeof($tmp_arr) - 1; $i++) {
		...
		if ($v.$1 <= $header_lv) {
			_return.i = $i;
			_return.arr = array_slice($tokens_arr, $header_line_i + 1, $i);
			return _return;
		}
	}
	_return.i = $i;
	_return.arr = array_slice($tokens_arr, $header_line_i + 1);
	return _return;;
}
Mixed parse_array_element(tokens_line $tokens_arr) {
...
Mixed parse_array(tokens_line $tokens_arr) {
	arr_ll $all_commas;
	tokens_line_arr $elements;
	Mixed $return;
	tokens_line_arr $element;
	$all_commas = find_all_token_at_level0( { ",", "operator" }, $tokens_arr);
...
//return false;
//}
tokens_line remove_all_spaces(tokens_line $tokens_arr) {
	tokens_line $return;
	token $ele;
	$return = { };
	for (auto [$key_rtt, $ele] : $tokens_arr) {
...
Mixed parse_var(tokens_line $tokens_arr) {
...
Mixed parse_complex_var(tokens_line $tokens_arr) {
	arr_ll $all_dots_pos;
	tokens_line_arr $cutted;
	tokens_line $var;
	$all_dots_pos = find_all_token_at_level0( { ".", "operator" }, $tokens_arr);
...
Mixed process_assignment(tokens_line $tokens_arr) {
...
Mixed process_multiple_operators(tokens_line $tokens_arr) {
...
//	long $k;
	...
	while (true) {
		...
		if (sizeof($arr_operators) == 1) {
			_tmp.push("type", "expr");
			_tmp.push("body", tokens_arr_to_name_and_type_format($tokens_arr));
			return _tmp;
		}
		$min = 30;
		...
		if (__real_operators[$highest_operator][0] == 0) {
			$return = array_slice($tokens_arr, 0, $highest_operator_pos);
			_tmp.push("type", "expr");
			_tmp1.push(0, $tokens_arr[$highest_operator_pos]);
			_tmp1.push(1, $tokens_arr[$highest_operator_pos + 1]);
			_tmp.push("body", _tmp1);
			$tokens_arr_tmp = array_slice($tokens_arr,
					$highest_operator_pos + 2, sizeof($tokens_arr));
			$return = array_merge($return, $tokens_arr_tmp);
		} else if (__real_operators[$highest_operator][0] == 1) {
			$return = array_slice($tokens_arr, 0, $highest_operator_pos - 1);
			_tmp = Mixed();
			_tmp.push("type", "expr");
			_tmp1 = Mixed();
			_tmp1.push(0, $tokens_arr[$highest_operator_pos - 1]);
			_tmp1.push(1, $tokens_arr[$highest_operator_pos]);
			_tmp.push("body", _tmp1);
			$return.push(_tmp);
			$tokens_arr_tmp = array_slice($tokens_arr,
					$highest_operator_pos + 1, sizeof($tokens_arr));
			$return = array_merge($return, $tokens_arr_tmp);
			...
			$return = array_merge($return,
					array_slice($tokens_arr, $highest_operator_pos + 2));
		}
		$tokens_arr = $return;
	}
	_tmp = Mixed();
...
bool is_new_clause(tokens_line $tokens_arr) {
	token $token;
	$tokens_arr = remove_all_spaces($tokens_arr);
...
Mixed tokens_arr_trim1(Mixed $tokens_arr) {
	//reset($tokens_arr);
	if (isset($tokens_arr, sizeof($tokens_arr) - 1)
			&& $tokens_arr[(long) sizeof($tokens_arr) - 1][1l] == "spaces")
		array_pop_nr($tokens_arr);
	if (isset($tokens_arr, 0l) && $tokens_arr[0][1] == "spaces") {
		array_shift_nr($tokens_arr);
//		reset($tokens_arr);
	}
	return $tokens_arr;
}
Mixed tokens_arr_trim(Mixed $tokens_arr) {
...
Mixed convert_spaces_to_concat_operators(Mixed $tokens_arr) {
	arr_ll $spaces_poses;
	Mixed $return;
//	long $pos;
	token $token;
...
// $spaces_poses = find_all_token_at_level0_by_type ( "spaces", $tokens_arr );
	for (auto [$pos, $token] : $tokens_arr.valueArray) {
		$type = (string) $token[1];
		if ($type != "spaces") {
			$return.push($token);
			continue;
...
Mixed parse_expression(Mixed $tokens_arr) {
...
string $type;
string $name;
if ($tokens_arr["type"] == "expr")
	$tokens_arr = (Mixed) $tokens_arr["data"];
$complex_expr_arr = Mixed();
//	reset($tokens_arr);
$tokens_arr = tokens_arr_trim1($tokens_arr);
if (sizeof($tokens_arr) == 1) {
	if (is_token(get_1st_ele($tokens_arr))) {
		if (in_array((string) get_1st_ele($tokens_arr)[1], { { 0, "string" }, {
				1, "number" }, { 2, "var" }, { 3, "class" }, { 4, "array" }, {
				5, "expr" } })) {
...
for (long $k = 0l; $k < (long) sizeof($tokens_arr); $k++) {
	$token = $tokens_arr[$k];
	if ($token.$0 != "") {
	...
	if ($token.$0 == "(" && $token.$1 == "operator") {
		...
		$result_arr = get_arr_between_brackets_mixed("(", $tokens_arr, $k);
...
// assignment expr
for (auto [$k, $token] : $tokens_arr.valueArray) {
	if ($token[0] == ":" && $token[1] == "operator") {
...
$return.push("body", tokens_arr_to_name_and_type_format($tokens_arr));
return $return;
}
Mixed parse_expression(tokens_line $tokens_arr) {
return parse_expression(Mixed($tokens_arr));
}
Mixed structurelize(tokens_arr _arr);
Mixed process_if(long &$line_num, tokens_arr $tokens_arr) {
tokens_line $if_line;
getline_subblock_t $getline_subblock;
map<long, tokens_arr> $if_blocks;
map<long, Mixed> $if_conditions;
tokens_arr $else_block;
Mixed $return, _tmp;
...
while (true) {
	if (isset($tokens_arr[$line_num + 1], 0)
			&& $tokens_arr[$line_num + 1].$0[0].$0 == "elseif"
			&& $tokens_arr[$line_num + 1].$0[0].$1 == "keyword") {
		$line_num++;
...
if ($tokens_arr[$line_num + 1].$0[0].$0 == "else"
		&& $tokens_arr[$line_num + 1].$0[0].$1 == "keyword"
		&& sizeof($tokens_arr[$line_num + 1].$0) == 1) {
	$line_num++;
	$getline_subblock = getline_subblock($tokens_arr, $line_num);
	$line_num += $getline_subblock.i;
...
if ($for_tokens[2].$1 == "number" && isset($for_tokens, 4)
	...
	$return.push(tokenize($for_tokens[4].$0 + "<=" + $for_tokens[2].$0)[0].$0);
...
Mixed process_for(long &$line_num, tokens_arr $tokens_arr) {
tokens_line $for_line;
getline_subblock_t $getline_subblock;
tokens_arr $for_block;
Mixed $for_parse;
Mixed $return;
...
Mixed process_while(long $line_num, tokens_arr $tokens_arr) {
tokens_line $while_line;
Mixed $return;
//	long $while_lv;
tokens_arr $while_block;
getline_subblock_t $getline_subblock;
Mixed $for_parse;
...
// throw  ( "non-var not allowed function declare" );
		...
		$arr_between_brackets = get_arr_between_brackets_tl("[", $tokens, $i);
...
Mixed process_function_def(long $line_num, tokens_arr $tokens_arr,
	Mixed $func_declare, bool $is_method = false) {
getline_subblock_t $getline_subblock;
tokens_arr $func_def_block;
Mixed $return, _tmp;
$getline_subblock = getline_subblock($tokens_arr, $line_num);
$line_num += $getline_subblock.i;
...
Mixed parse_property(tokens_line $tokens_arr) {
arr_ls $modifiers;
...
Mixed is_method_def(tokens_line $tokens_arr) {
arr_ls $modifiers;
...
Mixed parse_class(tokens_arr $class_def_block) {
Mixed $return;
unsigned long $i;
tokens_line1 $line;
Mixed $result;
...
Mixed process_class_def(long $line_num, tokens_arr $tokens_arr,
	Mixed $class_name) {
getline_subblock_t $getline_subblock;
tokens_arr $class_def_block;
Mixed _tmp;
$getline_subblock = getline_subblock($tokens_arr, $line_num);
$line_num += $getline_subblock.i;
...
Mixed is_catch_line(long $line, tokens_arr $tokens_arr) {
Mixed $return;
...
bool is_finally_line(long $line, tokens_arr $tokens_arr) {
tokens_line $line_tokens;
$line_tokens = remove_spaces_from_tokens_arr($tokens_arr[$line].$0);
...
Mixed process_try(long $line, tokens_arr $tokens_arr) {
Mixed $return;
long $tabs_lv;
Mixed $blocks_type, _tmp, _tmp1;
long $block_num;
map<long, tokens_arr> $blocks;
long $i;
...
$block_num = 0;
_tmp = Mixed();
for ($i = $line + 1; $i < (long) sizeof($tokens_arr); $i++) {
	if ($tokens_arr[$i].$1 < $tabs_lv)
		break;
	if ($tokens_arr[$i].$1 == $tabs_lv
			&& ($catch_line_parsed = is_catch_line($i, $tokens_arr))) {
// $catch_line = remove_all_spaces ( $tokens_arr [$i] .$0 );
		$block_num++;
		$blocks_type.push($catch_line_parsed);
	} else if ($tokens_arr[$i].$1 == $tabs_lv
			&& is_finally_line($i, $tokens_arr)) {
		$block_num++;
		$blocks[$block_num] = { };
		_tmp = Mixed();
		_tmp.push("finally");
...
Mixed process_defcat(long $line, tokens_arr $tokens_arr) {
long $tabs_lv;
tokens_arr $defcat;
tokens_arr $deffin;
bool $is_in_deffin;
long $i;
map<long, tokens_arr> $defcat_blocks;
...
Mixed structurelize(tokens_arr $tokens_arr) {
Mixed $return, _tmp, _tmp1;
Mixed $defcat_rs;
long $line;
...
for ($line = 0; $line < (long) sizeof($tokens_arr); $line++) {
...
tokens_arr merge_strings(tokens_arr $tokens_arr) {
tokens_arr $return;
//	long $k;
...
token $token;
$return = { };
for (auto [$k, $tokens_line] : $tokens_arr) {
	_tmp.$0 = { };
	_tmp.$1 = $tokens_arr[$k].$1;
	$return[$k] = _tmp;
	$current_str = "";
...
if ($input.contains("name") && $input.contains("type")
		&& $input["name"] == $obj["name"] && $input["type"] == $obj["type"]) {
	return true;
...
int main() {
long _len;
for (auto [_key, _operator] : __operators) {
	if (array_search(php_to_string(_operator.at(0)), __operator_1st_chars)
			== false)
		array_push(__operator_1st_chars, php_to_string(_operator.at(0)));
	_len = strlen(_operator);
	if (array_search(_operator, __operators_by_chars_count[_len]) == false)
